...
class FakeUserAgentMiddleware:
    ...
    def __init__(self, settings):
        ...
        self.scrapeops_endpint = settings.get('SCRAPEOPS_FAKE_USER_AGENT_ENDPOINT','http://headers.scrapeops.io/v1/user-agents?')
    ...
    def _get_user_agents_list(self):
        ...
        response = requests.get(self.scrapeops_endpint, params=urlencode(payload))
    ...
    def _scrapeops_fake_user_agents_enabled(self):
        if self.scrapeops_api_key is None or self.scrapeops_api_key == '' or self.scrapeops_fake_user_agents_active == False:
            self._scrapeops_fake_user_agents_active = False
        else:
            self._scrapeops_fake_user_agents_active = True

    def process_request(self, request, spider):
        random_user_agent = self._get_random_user_agent()
        request.headers['User-Agent'] = random_user_agent
        print("****USER AGENTS ATTACHED****")
        print(request.headers["User-Agent"])


class FakeBrowserHeadersMiddleware:
    ...
    def __init__(self, settings):
        self.scrapeops_api_key =  settings.get('SCRAPEOPS_API_KEY')
        self.scrapeops_endpint = settings.get('SCRAPEOPS_FAKE_BROWSER_HEADER_ENDPOINT','https://headers.scrapeops.io/v1/browser-headers')
